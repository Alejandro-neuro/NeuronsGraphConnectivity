...
def Draw2Graph( G1,G2,pos = None, styleDark = False ):
    ...
    nx.draw_networkx_edges(G1, pos, edge_color=edge_color1, alpha=1.0, width=3.0,connectionstyle="arc3,rad=0.1", arrowsize=25, node_size = 2000, ax=axarr)
    ...
    nx.draw_networkx_labels(G1, pos, font_size=30, font_color= text_color, font_family='Georgia', font_weight='normal', alpha=None, bbox=None, horizontalalignment='center', verticalalignment='center', ax=axarr)
    

    plt.text(-10, 25, "$\longrightarrow$ Predicted", size=40, rotation=0.,
         ha="left", va="center", color = edge_color2)
    plt.text(-10, 27, "$\longrightarrow$ Real", size=40, rotation=0.,
